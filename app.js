/**
 * ClipTuck - 後で読むアプリ（体験版）
 * ローカルストレージベースのMVP実装
 * 
 * ⚠️ 重要：この体験版は固定版です ⚠️
 * - このファイルは今後変更されません
 * - 新機能やUI改善は完全版（full/）のみに適用されます
 * - 体験版のバージョン：v1.0.0-trial-locked
 * - 固定日：2024年1月
 */

const MAX_BOOKMARKS = 50;
const VERSION = 'trial';
const VERSION_LOCKED = 'v1.0.0-trial-locked';
const LOCKED_DATE = '2024-01-01';
...
// 上限チェック関数
function checkBookmarkLimit() {
    if (appState.bookmarks.length >= MAX_BOOKMARKS) {
        showToast(`体験版は${MAX_BOOKMARKS}件まで保存できます。完全版にアップグレードしてください。`, 'error');
        return false;
    }
    return true;
}
